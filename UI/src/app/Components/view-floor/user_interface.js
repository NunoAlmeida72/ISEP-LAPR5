...
export default class UserInterface extends GUI {
    constructor(thumbRaiser,maps,tasks,robots) {
        super();
        ...
        const building = ["assets/mazes/ckarzx_20x20_displacement.json","assets/mazes/test.json", "assets/mazes/ckarzx_20x20.json", "assets/mazes/Loquitas_10x10_displacement.json", "assets/mazes/Loquitas_10x10.json", "assets/mazes/Loquitas_20x20_displacement.json", "assets/mazes/Loquitas_20x20.json"];

        const floors=[];
        for(let i=0;i<maps.length;i++){
            floors.push(maps[i].name);
        }

        const floorsSelectionFolder=this.addFolder("Floors");
        floorsSelectionFolder.add(thumbRaiser.json, "map").options(floors);
        floorsSelectionFolder.add({ Confirm: async () => await thumbRaiser.changeFloor() }, "Confirm");
        floorsSelectionFolder.close();

        
        const robots1=[];
        for(let i=0;i<robots.length;i++){ 
            robots1.push({toString: function () {
        ...
        var automaticTasksSelectionFolder=this.addFolder("Tasks");
        
        function updateTasksList(robotName) {
            automaticTasksSelectionFolder.controllers.find(ctrl => ctrl.property === "Confirm").hide();
            tasks1=[];
            if(robotName!=""){
                for (let i = 0; i < tasks.length; i++) {
                    if (tasks[i].assignedTo === robotName.value ) {
                        tasks1.push({toString: function () {
                            return tasks[i].name; 
                        },value:tasks[i].id});
                    }
                }
            }
            thumbRaiser.json.task="";
            automaticTasksSelectionFolder.controllers.find(ctrl => ctrl.property === "task").show().options(tasks1).onChange(allowConfirm);
        }

        function allowConfirm(task){
            if(task!=""){
                automaticTasksSelectionFolder.controllers.find(ctrl => ctrl.property === "Confirm").show();
            }
        }

        automaticTasksSelectionFolder.add(thumbRaiser.json, "robot").options(robots1).onChange(updateTasksList);
        automaticTasksSelectionFolder.add(thumbRaiser.json, "task").options(tasks1).hide();
        automaticTasksSelectionFolder.add({ Confirm: () => thumbRaiser.animateRobot() }, "Confirm").hide();
        automaticTasksSelectionFolder.close();

        var elevatorFloorFolder=this.addFolder("Elevator Floors");
        
        var maps1 = [];

        elevatorFloorFolder.add(thumbRaiser.json, "map").options(maps1).hide();
        elevatorFloorFolder.add({ Confirm: async () => await thumbRaiser.changeElevatorFloor() }, "Confirm").hide();
        elevatorFloorFolder.close();
        /*
        const buildingSelectionFolder = this.addFolder("Building");
        //buildingSelectionFolder.add(thumbRaiser.mazeParameters, "url").name("Building"); 

        buildingSelectionFolder.add(thumbRaiser.mazeParameters, "url").options(building);
        buildingSelectionFolder.add({ Confirm: () => thumbRaiser.changeBuilding() }, "Confirm");
    ...
    chooseFloor(floors, maps) {
        if (this.folders[2].controllers.find(ctrl => ctrl.property == "map")._hidden) {
            var maps1 = [];
            for (let i = 0; i < floors.length; i++) {
                for (let j = 0; j < maps.length; j++) {
                    if (floors[i] == maps[j].floorId) {
                        maps1.push(maps[j].name);
                    }
                }
            }
            this.folders[2].controllers.find(ctrl => ctrl.property == "map").show().options(maps1);
            this.folders[2].controllers.find(ctrl => ctrl.property == "Confirm").show();
            this.folders[2].open();
            this.open();
        }
    }
    ...
    dontShow() {
        this.folders[2].controllers.find(ctrl => ctrl.property == "map").hide();
        this.folders[2].controllers.find(ctrl => ctrl.property == "Confirm").hide();
    }
}